...
<template>
    <div class="order-pay-page" v-if="orderPre && address">
        ...
        <van-submit-bar :price="orderPre.actualPayment * 100" button-text="立即支付" button-type="primary" text-align="left"
            @click="submit" :loading="loading"></van-submit-bar>
        <cp-pay-sheet :orderId="orderId" :actualPayment="orderPre.actualPayment" payCallback="http://localhost/order/pay/result"
            v-model:show="show" />
    </div>
